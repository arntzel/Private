...
-(NSArray *) getPendingFeedEventEntitys
{
    ...
    NSSortDescriptor *sortDescriptor = [[NSSortDescriptor alloc] initWithKey:@"last_modified" ascending:YES];
...
- (FeedEventEntity *)getFeedEventWithEventType:(int)eventType WithExtEventID:(NSString *)ext_event_id
{
    ...
    predicate = [NSPredicate predicateWithFormat:@"(eventType = %d) && (ext_event_id=%@)", eventType, ext_event_id];
    [fetchRequest setPredicate:predicate];
    NSArray * results = [managedObjectContext executeFetchRequest:fetchRequest error:nil];
    if ([results count] > 0)
    {
