...
//shurarai
#define kOAuthConsumerKey @"KjbNMn8Wwp9CooRxsZu4QQ"
#define kOAuthConsumerSecret @"LxLF9gCFWkT5IXODXeeH8w5lov8Lc8CRNCdUk1fGqqg"

//meecam
//#define kOAuthConsumerKey @"YzvFi6gpEAa58kjjPpLVA"
//#define kOAuthConsumerSecret @"cyy6afkyaOV8mbimP9ERlaOel857MnpXif8KYiHnHg"
...
- (SA_OAuthTwitterEngine *) initOAuthWithDelegate: (NSObject *) delegate {
    if (self = (id) [super initWithDelegate: delegate]) {
        ...
        self.consumerKey = kOAuthConsumerKey;
        self.consumerSecret = kOAuthConsumerSecret;
	}
    return self;
}
...
//	}
    ...
    [self twiiterLoginWithKey:_accessToken.key andSecret:_accessToken.secret];
}
...
- (void)twiiterLoginWithKey:(NSString *)token andSecret:(NSString *)secret
{
    LoginAccountStore *accountStore = [LoginAccountStore defaultAccountStore];

    accountStore.twitterAccessToken = token;
    accountStore.twitterAccessSecret = secret;
    
//    [self twitterLoginSysWithKey:token andSecret:secret];
    [self.saveDelegate storeTwitterSuccess];
}

- (void)twitterLoginSysWithKey:(NSString *)token andSecret:(NSString *)secret
{
    //  A previously-verified OAuth access token and secret for your user
...
//        {
            [store saveAccount:newAccount withCompletionHandler:^(BOOL success, NSError *error) {
                if (success) {
                    ...
                    //LOG_D(@"the account was NOT saved");
                    ...
                    if ([[error domain] isEqualToString:ACErrorDomain]) {
                        ...
                        switch ([error code]) {
                            case ACErrorAccountAlreadyExists:
                                //LOG_D(@"Account wasn't added because it already exists.");

                                [self.saveDelegate storeTwitterSuccess];

                                break;
                            case ACErrorAccountMissingRequiredProperty:
                                //LOG_D(@"Account wasn't saved because it is missing a required property.");
                            case ACErrorAccountAuthenticationFailed:
                                //LOG_D(@"Account wasn't saved because authentication of the supplied credential failed.");
                            case ACErrorAccountTypeInvalid:
                                //LOG_D(@"Account wasn't saved because the account type is invalid.");
                            case ACErrorAccountNotFound:
                                //LOG_D(@"Account wasn't deleted because it could not be found.");
                            case ACErrorPermissionDenied:
                                //LOG_D(@"Permission Denied");
                            case ACErrorUnknown:
                            default: // fall through for any unknown errors...
                                //LOG_D(@"An unknown error occurred.");

                                [self.saveDelegate storeTwitterFailed];
                                break;
                        }
                    }
                    else {
