...
@protocol SA_OAuthTwitterSaveToSysDelegate
@optional
- (void) storeTwitterSuccess;				//implement these methods to store off the creds returned by Twitter
- (void) storeTwitterFailed;									//if you don't do this, the user will have to re-authenticate every time they run
@end
...
@interface SA_OAuthTwitterEngine : MGTwitterEngine {
	...
	NSURL		*_authorizeURL;
    id <SA_OAuthTwitterSaveToSysDelegate> saveDelegate;
...
@property (nonatomic, assign) id <SA_OAuthTwitterSaveToSysDelegate> saveDelegate;
